...
public class LikeReviewServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(request.getParameter("like_review") != null){
            try {
                reviewService.likeReview(user.getUser_id(), review_id);
            } catch (SQLException throwables) {
                throwables.printStackTrace();
            }
        }
        if(request.getParameter("unlike_review") != null){
            try {
                reviewService.unlikeReview(user.getUser_id(), review_id);
            } catch (SQLException throwables) {
                throwables.printStackTrace();
            }
        }
